    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div class="space-y-2">
        ...
        <p class="max-w-2xl text-sm leading-6 text-toned">
          CSV працює тільки з колонками <code>term</code> і <code>definition</code>.
          Імпорт повністю замінює поточний список карток у формі.
        </p>
      </div>
...
const exportCsv = () => {
  const rows = [
    ["term", "definition"],
    ...props.cards.map((card) => [card.term ?? "", card.definition ?? ""]),
  ];
...
const onFileChange = async (event: Event) => {
  ...
  try {
    ...
    if (importedRows.length === 0) {
      throw new Error("CSV не містить жодного рядка з term і definition.");
    }
...
const parseCsv = (input: string): CsvRow[] => {
  ...
  if (termIndex === undefined || definitionIndex === undefined) {
    throw new Error("CSV має містити заголовки term і definition.");
  }
  ...
  return dataRows
    ...
    .filter((row) => row.term.length > 0 || row.definition.length > 0);
};
