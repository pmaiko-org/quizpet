...
`shared` — это не отдельная папка, а набор корневых каталогов `app/`: `components/` (`base/` — доменно-независимые презентационные компоненты с префиксом `Base*`: от атомов-обёрток до переиспользуемых секций; `layout/` — хром приложения с префиксом `The*`, см. ниже), `composables/`, `utils/`, `validation.ts`, `store/` (глобальные Pinia-сторы; сейчас пуст), `repository/`, `types/`.
...
> Хром приложения (`TheHeader`/`TheFooter`/`TheSidebar`) — это обвязка макета, а не виджет страницы; живёт в `components/layout/` (shared) и монтируется в `layouts/`.

## Layout-компоненты (`components/layout/`, префикс `The*`)
...
```
components/layout/          # The* — по одному на макет
├── TheHeader.vue
├── TheFooter.vue
├── TheSidebar.vue
...
Правило: **`The*` = используется единожды и является составной частью лейаута.** Это НЕ относится к переиспользуемым презентационным компонентам из `components/base/` (`BaseHeroSection`, `BaseMetricGrid`, `BaseSectionHeader`, …) — те доменно-независимы, могут монтироваться многократно на любых страницах и живут под префиксом `Base*`.
